        ...
        <div className="bottom-0 w-full left-0 fixed gap-10 bg-black md:flex md:items-center" >   {/* contains everything */}
            ...
            <div className="  p-2 w-full" >   {/** seek bar and controller's div */}
                ...
                <div className=" cursor-pointer flex items-center justify-center gap-5 " >   {/* controllers */}
                    <SkipBackIcon onClick={BackwardSong} className=" fill-[#c2c2c2dd] text-[#a7a7a7] h-6 w-6" />
                    {/* <PlayIcon onClick={TogglePlayer} className={''} /> */}
                    {
                        play ? < PauseIcon className="  fill-white text-white h-8 w-8" onClick={TogglePlayer} /> : < PlayIcon className="  fill-white text-white h-8 w-8" onClick={TogglePlayer} />
                    }
                    <SkipForward onClick={ForwardSong} className=" fill-[#c2c2c2dd] text-[#a7a7a7] h-6 w-6" />
                </div>

                <div className=" flex text-white gap-2 p-2  justify-center " >{/* seek bar div */}
                    <h1 ref={current_time_stamp} className="text-[14px] text-gray-400" >0:00</h1>   {/* duration */}
                    <input ref={Seek_bar} onChange={SeekBar} type="range" max={Audioref.current.duration} className="range w-100" />
                    <h1 ref={duration_stamp} className="text-[14px] text-gray-400" >0:00</h1>   {/* current time */}
                </div>
            </div>
        </div>
    )
