...
export default function Playerdiv(props) {
  ...
  const ForwardSong = () => {
    ...
    if (currentindex >= props.popular_songs.length) {
      // checks if index is > response's length
      setIndex(0);
    } else {
      // forwards the song
      setIndex(currentindex + 1);
      song_name_section.current.textContent =
        props.popular_songs[currentindex].song_name;
      artist_name_section.current.textContent =
        props.popular_songs[currentindex].artist;
  ...
  // auto plays the song on every backward/forward skip
  useEffect(() => {
  ...
  return (
    <div className="bottom-0 w-full left-0 fixed gap-10 bg-black md:flex md:items-center">
      {" "}
      {/* contains everything */}
      <div className="flex items-center gap-2  w-full ">
        {" "}
        {/** poster and song data div */}
        <div className="p-3">
          {" "}
          {/* img div */}
          <img
            ref={Poster_ref}
            src={`http://127.0.0.1:7000${props.song_data.poster}/`}
        ...
        <div className="  capitalize ">
          {" "}
          {/* song info div */}
          <h1 ref={song_name_section} className="text-white text-[14px]">
            {props.song_data.song_name
              ? props.song_data.song_name
              : "song name"}
          </h1>
          <h1 ref={artist_name_section} className="text-gray-400 text-[12px]">
            {props.song_data.artist ? props.song_data.artist : "artist name"}
          </h1>
        </div>
      </div>
      <div className="  p-2 w-full">
        {" "}
        {/** seek bar and controller's div */}
        {/* audio tag */}
        <audio
          onEnded={HandleEnd}
          onTimeUpdate={Format_time}
          onLoadedMetadata={Format_time}
          ref={Audioref}
          src={
            `http://127.0.0.1:7000/${props.song_data.src}/`
              ? `http://127.0.0.1:7000/${props.song_data.src}/`
              : ""
          }
        ></audio>
        <div className=" cursor-pointer flex items-center justify-center gap-5 ">
          ...
          <SkipBackIcon
            ...
            className=" fill-[#c2c2c2dd] text-[#a7a7a7] h-6 w-6"
          />
          {/* <PlayIcon onClick={TogglePlayer} className={''} /> */}
          {play ? (
            <PauseIcon
              className="  fill-white text-white h-8 w-8"
        ...
        <div className=" flex text-white gap-2 p-2  justify-center ">
          {/* seek bar div */}
          <h1 ref={current_time_stamp} className="text-[14px] text-gray-400">
            0:00
          </h1>{" "}
          {/* duration */}
          <input
          ...
          />
          <h1 ref={duration_stamp} className="text-[14px] text-gray-400">
            0:00
          </h1>{" "}
          {/* current time */}
        </div>
