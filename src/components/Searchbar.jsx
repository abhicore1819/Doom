...
import { FileTerminal, FileTypeCorner, RadioTower, SearchIcon } from "lucide-react";
...
export default function Searchbar() {
    ...
    return (
        <div className="bg-gray-900 min-h-screen p-5 text-white" >
            ...
            <form onSubmit={HandleSubmit} className="p-5 flex justify-center " >   {/* search bar div */}
                <input value={query} onChange={Handlequery} type="text" className=" outline-none bg-gray-700 md:w-1/2 lg:w-1/3 rounded-l-full p-2 w-full" placeholder="What do you want to listen?" />
                <SearchIcon onClick={HandleSubmit} className="bg-gray-700 h-13 w-13 p-2 rounded-r-full" />
            </form>
            <div className=" capitalize space-y-5 flex flex-col">
                {show_response ? displaysongs.map((element, index) => (
                    // contains the song info and img div
                    <div onClick={(() => setSongData({ song_name: element['song_name'], artist: element['artist'], poster: element['poster'], src: element['song_url'] }))} className=" gap-2 flex max-w-1/2 rounded-2xl p-2 bg-linear-to-r from bg-gray-700 via-gray-600 to-gray-800 space-y-5" key={index} >
                        <div className="p-2 h-28 w-28" >   {/* contains the img */}
                            <img className="rounded-2xl" src={`http://127.0.0.1:7000/${element['poster']}`} alt="" />
                        </div>
                        <div className="p-2" >
                            {/* song info div */}
            ...
            <div className=" flex flex-col items-center gap-5 p-5" >
                <h1 className={` ${isresponseempty ? 'text-red-500 text-center text-2xl' : 'opacity-0'}`} > {isresponseempty ? "We could'nt find !" : ""}</h1>
                {isresponseempty &&
                    <button className="bg-linear-to-r from bg-blue-700 via-blue-800 to-blue-950 p-2 rounded-2xl" >Request this song !</button>}
            </div>
            {/* ----- req btn and empty res err div */}
